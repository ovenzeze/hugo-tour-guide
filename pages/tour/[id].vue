<template>
  <div class="h-full w-full max-h-full relative bg-gray-50" v-motion :initial="{ opacity: 0 }"
    ...
    <div class="absolute inset-0 pt-16 pb-16 z-0">
    ...
    <div class="absolute top-0 left-0 right-0 z-10 bg-white bg-opacity-90 backdrop-blur-sm p-3 shadow-md">
      <div class="flex justify-between items-center max-w-screen-xl mx-auto">
        <div class="flex items-center">
          <button class="text-blue-500 p-1 mr-2 rounded-full hover:bg-blue-50" @click="goBack">
        ...
        <div class="flex space-x-2">
          ...
          <div class="flex space-x-1 items-center">
            <button
              ...
              :class="currentFloorStr === floor ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'"
    ...
    <!-- 右侧面板 - 可折叠的推荐路线 -->
    <div 
      class="absolute top-16 right-0 bottom-16 z-10 w-80 bg-white shadow-lg rounded-l-lg transform transition-transform duration-300 flex flex-col"
      :class="showRoutePanel ? 'translate-x-0' : 'translate-x-full'"
    >
      <div class="p-3 border-b flex justify-between items-center">
        <h2 class="text-base font-semibold text-gray-900">Recommended Route</h2>
        <button class="text-gray-500 p-1 rounded hover:bg-gray-100" @click="showRoutePanel = false">
      ...
      <div class="overflow-auto flex-1">
        <div class="divide-y">
          <div v-for="(item, index) in filteredRouteItems" :key="item.id"
            ...
            <!-- 项目信息 -->
            <div class="flex-1">
              <div class="font-medium text-gray-900 mb-1 text-sm">{{ item.name }}</div>
    ...
    <!-- 展品详情面板 - 更小的底部抽屉，不会完全覆盖地图 -->
    <div 
      v-if="selectedExhibit"
      class="absolute bottom-16 left-0 right-0 z-20 bg-white shadow-lg rounded-t-lg transform transition-transform duration-300"
      :class="showExhibitPanel ? 'translate-y-0' : 'translate-y-full'"
      style="max-height: calc(40vh);"
    >
      <div class="p-3 border-b flex justify-between items-center">
        <h2 class="text-base font-semibold text-gray-900 truncate">{{ selectedExhibit.name }}</h2>
        <button class="text-gray-500 p-1 rounded hover:bg-gray-100" @click="showExhibitPanel = false">
          <span class="material-icons text-sm">expand_more</span>
        </button>
      </div>
      
      <div class="p-3 overflow-auto" style="max-height: calc(40vh - 50px);">
        <p class="text-gray-600 mb-3 text-sm">{{ selectedExhibit.description || 'No description available.' }}</p>
        
        <div class="flex gap-3 snap-x snap-mandatory overflow-x-auto hide-scrollbar pb-3">
          <div v-for="(_, i) in 3" :key="i" class="snap-start min-w-[200px] flex-shrink-0 bg-gray-100 rounded-lg aspect-video flex items-center justify-center">
            <span class="text-gray-500 text-sm">展品图片 {{ i + 1 }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="fixed bottom-20 right-4 z-30 flex flex-col gap-2">
      <button 
        class="w-10 h-10 rounded-full bg-white shadow-lg flex items-center justify-center text-blue-500 hover:bg-blue-50"
      ...
      <button 
        class="w-10 h-10 rounded-full bg-green-500 shadow-lg flex items-center justify-center text-white hover:bg-green-600"
...
// 处理展品选择
function onExhibitSelected(exhibit) {
  selectedExhibit.value = exhibit
  showExhibitPanel.value = true
  
  // 只有在用户实际点击展品时才解释
  if (exhibit && exhibit.userInteraction === true) {
    // 使用语音导航解释展品
